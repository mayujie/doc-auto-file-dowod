...
def make_test_proof_back_page(
    ...
    registration_proof_info = RegistrationProofInfo(
        ...
        section_E=SectionInfo(
            ...
            position=(img_size[1] // 6, int(img_size[0] // 2.8))
        ),
    ...
    if debug:
        ...
        save_dir = os.path.join(os.path.dirname(__file__), 'results_dowod_front')
        os.makedirs(save_dir, exist_ok=True)
        output_image_path = os.path.join(
            save_dir,
            f'frontpage_{car_info.maker}_{car_info.model}_{car_info.vin_number}_{car_info.plate_number}.png'
        )
        cv2.imwrite(output_image_path, img)


if __name__ == '__main__':
    IMG_PATH = "assets/Template_front.png"
    DEBUG = False
